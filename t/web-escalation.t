...

{
    ...
    $t->get_ok('/search')->status_is(200)
        ->element_exists_not( 'a#nav-escalation', 'no navbar link when disabled' );
...
# ---------------------------------------------------------------------------
# 2.  Enabled: navbar link, management page, and masking
# ---------------------------------------------------------------------------

{
    ...
    $t->get_ok('/search')->status_is(200)
        ->element_exists( 'a#nav-escalation', 'navbar link present when enabled' );

    no warnings qw(redefine once);
    local *Lilith::escalation_targets = sub {
        return [
            {   id          => 1,
                name        => 'soc-hook',
                type        => 'Webhook',
    ...
    my $body = $t->get_ok('/escalation')->status_is( 200, 'management page renders' )
        ...
        ->content_like( qr/soc-hook/, 'target name shown' )->tx->res->body;
    unlike( $body, qr/sekritvalue/, 'secret config value never reaches the browser' );

    # targets API masks secrets and flags them via secrets_set
    ...
    # types API drives the dynamic config form
...

{
    ...
    $t->post_ok( '/api/escalation/targets', form => { a => 'b' } )
        ->status_is( 400, 'non-JSON body is a 400' );
}
...

{
    ...
    $t->post_ok('/api/escalation/targets/7/test')->status_is( 200, 'test renders 200' )
        ->json_is( '/ok', 1 )
        ->json_is( '/payload/message', 'test payload for 7', 'test payload streamed back' );
    $t->post_ok('/api/escalation/targets/99/test')->status_is( 502, 'failed test is a 502' )
...

{
    ...
    $t->get_ok('/api/escalation/history/suricata/9')->status_is( 200, 'history renders 200' )
...

{
    ...
    $t->get_ok('/event/suricata/42')->status_is(200)
    ...
    $t->get_ok('/search?search=1&table=suricata')->status_is(200)
        ->element_exists( 'span.esc-badge', 'escalated badge shown in search results' )
        ->text_like( 'span.esc-badge', qr/E/, 'badge is the E marker' )
        ->element_exists( 'span.esc-badge[title="escalated 2 times"]',
        'badge count comes from the escalations array' );
    ...
    $t->get_ok('/search?search=1&table=suricata&partial=1')->status_is(200)
