...

{
	...
	$t->get_ok('/search')
		->status_is( 200, 'GET /search renders' )
		->element_exists_not( 'a#nav-logs', 'no Logs nav entry when Allani is not configured' );

	$t->get_ok('/logs')
		->status_is( 200, 'GET /logs renders even when not configured' )
...

{
	...
	$t->get_ok('/logs')
		->status_is( 200, 'GET /logs renders when configured' )
	...
	my $part = $t->get_ok('/logs?source=syslog&partial=1')->status_is( 200, 'partial render 200' )->tx->res->body;
	...
	$t->get_ok('/logs/syslog/1')
		->status_is( 200, 'record view renders' )
...

{
	...
	# http_all-by-client link
	{
		...
		$t->get_ok('/event/suricata/9')
			->status_is( 200, 'event view renders with Allani configured' )
	...
	# not configured: no Logs dropdown
	{
		my $t = _app();
		$t->get_ok('/event/suricata/9')
			->status_is( 200, 'event view renders without Allani' )
...

{
	...
	local *Lilith::Allani::top
		= sub { my ( $s, %o ) = @_; %top_opts = %o; return [ { value => 'sshd', count => 10 } ] };
	...
	$t->get_ok('/api/logs/top?source=http&column=vhost&measure=bytes')
		->status_is(200)
	...
	$t->get_ok('/api/logs/top?source=syslog&column=program&start=2026-07-18+00:00&end=2026-07-18+12:00')
		->status_is(200);
	...
	$t->get_ok('/api/logs/timeseries?source=syslog&bucket=hour')
		->status_is(200)
	...
	$t->get_ok('/api/logs/timeseries?source=syslog&group_by=program')
		->status_is(200)
		->json_is( '/grouped',      1,      'grouped timeseries is flagged grouped=1' )
		->json_is( '/rows/0/group', 'sshd', 'grouped rows carry the group value' );
	$t->get_ok('/api/logs/timeseries?source=syslog&bucket=auto&go_back_minutes=60')
		->status_is(200)
	...
	$t->get_ok('/api/logs/countries?source=syslog')
		->status_is( 200, 'countries endpoint renders' )
