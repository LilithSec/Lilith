...

{
	...
	my %bad = (
		...
		'unknown op'     =>
			{ match => { field => 'x', op => 'bogus', value => 1 }, actions => [ { escalate_to => ['soc'] } ] },
		'in without array' =>
			{ match => { field => 'x', op => 'in', value => 1 }, actions => [ { escalate_to => ['soc'] } ] },
		'bad regex' =>
			{ match => { field => 'x', op => 'regex', value => '(' }, actions => [ { escalate_to => ['soc'] } ] },
...

{
	...
	my $not = Lilith::AutoEscalate->compile(
		{ match => { not => { field => 'src_ip', op => 'in', value => ['1.2.3.4'] } } } );
...

{
	my $high = {
		...
		stop_on_match => 1,
		rule          =>
			{ match => { field => 'malscore', op => '>=', value => 8 }, actions => [ { escalate_to => ['soc'] } ] },
	};
	my $any_cape = {
		id            => 2,
		name          => 'any',
		...
		stop_on_match => 0,
		rule          =>
			{ match => { field => 'malscore', op => '>=', value => 1 }, actions => [ { escalate_to => ['ir'] } ] },
	};
