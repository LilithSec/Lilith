...

{
	...
	$t->get_ok('/api/ipinfo/192.168.1.999%20evil')->status_is( 400, 'IP with space/injection chars returns 400' );

	$t->get_ok('/api/ipinfo/1.2.3.4.5.6.7.8%3Bcommand')->status_is( 400, 'URL-encoded injection in IP returns 400' );
}
...

{
	...
	$t->get_ok('/api/ipinfo/127.0.0.1')
		->status_is( 200, 'valid IPv4 returns 200' )
	...
	$t->get_ok('/api/ipinfo/127.0.0.1')->json_is( '/ip', '127.0.0.1', 'ip field echoes input' );
...
SKIP: {
		...
		# ::1 (loopback)
		$t->get_ok('/api/ipinfo/::1')->status_is( 200, 'IPv6 loopback returns 200' );
...

{
	...
	$t->get_ok('/api/domaininfo/has%20space')->status_is( 400, 'domain with space (url-encoded) returns 400' );

	$t->get_ok('/api/domaininfo/semi%3Bcolon')->status_is( 400, 'domain with semicolon (url-encoded) returns 400' );
}
...

{
	...
	$t->get_ok('/api/domaininfo/example.com')->json_is( '/domain', 'example.com', 'domain field echoes input' );
}
...

{
	...
	}; ## end *Lilith::Web::Controller::Api::_httpsinfo_gather = sub
	use warnings qw(redefine once);

	my $j = $t->get_ok('/api/httpsinfo/example.com?port=443')->status_is( 200, 'httpsinfo renders 200' )->tx->res->json;
...

{
	...
	is_deeply(
		$sum->{mechanisms},
		[ 'ip4:1.2.3.0/24', 'include:_spf.example.com', 'a', 'mx', '-all' ],
	...
	local *Lilith::Web::Controller::Api::_mailinfo_gather = sub {
		...
		return {
			...
			spf    => {
				record  => 'v=spf1 -all',
				summary => { all => 'fail' },
				( $ip ? ( ip => $ip, result => 'fail' ) : () )
			},
			dmarc =>
				{ record => 'v=DMARC1; p=reject', found_at => $domain, p => 'reject', rua => 'mailto:x@' . $domain },
			dkim => {
				( $selector ? ( selector => $selector ) : ( probed => 1 ) ),
				keys =>
					[ { selector => ( $selector // 'default' ), key_type => 'rsa', key_bits => 2048, revoked => 0 } ],
			},
		};
	}; ## end *Lilith::Web::Controller::Api::_mailinfo_gather = sub
	use warnings qw(redefine once);

	my $j
		= $t->get_ok('/api/mailinfo/example.com?ip=8.8.8.8&selector=default')
		->status_is( 200, 'mailinfo renders 200' )
		->tx->res->json;
