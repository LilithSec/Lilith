...

{
	...
	$t->get_ok('/auto_escalation')
		->status_is( 200, 'page renders' )
	...
	$t->post_ok(
		'/api/auto_escalation/preview' => json => {
			rule => {
				match   => { field => 'x', op => 'bogus', value => 1 },
				actions => [ { escalate_to => ['soc-hook'] } ]
			}
		}
...

{
	...
	$t->post_ok(
		'/api/auto_escalation/rules' => json => {
			name => 'c2-beacon',
			rule => {
				match   => { field => 'signature', op => 'regex', value => 'cobalt' },
				actions => [ { escalate_to => ['soc-hook'] } ]
			},
			tables        => ['suricata'],
			priority      => 100,
	...
	$t->post_ok(
		'/api/auto_escalation/rules' => json => {
			...
			name => 'high-malscore',
			rule => {
				match   => { field => 'malscore', op => '>=', value => 9 },
				actions => [ { escalate_to => ['soc-hook'] } ]
			},
			enabled => 0,
		}
	...
	$t->post_ok( '/api/auto_escalation/rules' => json =>
			{ name => 'bad', rule => { match => { field => 'malscore', op => '>=', value => 8 }, actions => [] } } )
	...
	$t->post_ok( '/api/auto_escalation/rules/1/toggle' => json => { enabled => 1 } )
		->status_is( 200, 'toggle renders 200' )
		->json_is( '/enabled', 1 );
