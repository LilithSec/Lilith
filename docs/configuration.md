...
| `local_networks`             | Array of CIDRs (a bare address counts as itself) naming your own networks, for the `src_locality`/`dest_locality` dashboard dimensions and search filters: an address inside any of them reads *Internal*, anything else *External*. Unset, the private/unroutable ranges (RFC 1918, loopback, link-local, CGNAT, and their v6 counterparts) stand in, so both work before it is tuned. An entry that does not look like an address or CIDR is ignored with a logged warning. |
