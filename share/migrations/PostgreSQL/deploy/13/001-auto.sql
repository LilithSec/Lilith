...
-- / %INET machinery reuse it; its subject (a non-IP offender) gets its own
-- column. As with the sibling tables, only the scalar fields worth filtering /
-- sorting / grouping by are promoted to columns; the nested detail (attack,
-- rule, found, marks, references, ...) stays in raw, reachable via raw->'...'.
